var Engine = function (server, options) {
  ...
  if (!Array.isArray(hosts[0])) { // turn it into an array of arrays
    hosts = [hosts];
  }

  this._ns = this._options.namespace || '';

  // connects to each host and stores it in a shard manager
  function connectToHosts(hosts, isPrimary) {
    var shardsList = [];
    hosts.forEach(function (host) {
      var client, subscriber;
      var h = host.host,
        p = Number(host.port) || port,
        options = { no_ready_check:true, parser:'javascript' };

      client = redis.createClient(p, h, options);
      if(host.password) { client.auth(host.password); }
      var db = host.db || this.DEFAULT_DATABASE;
      client.select(db);

      if (isPrimary) {
        subscriber = redis.createClient(p, h, options);
        if(host.password) { subscriber.auth(host.password); }
      ...
      shardsList.push({ shardName:shardName, shard:shard});
    });

    return new ShardManager(shardsList);
  }

  hosts.forEach(function (hosts, i) {
    self._shardManagers.push(connectToHosts(hosts, i === 0));
  });
...
Engine.prototype = {
  ...
  emptyQueue:function (clientId) {
    if (!this._server.hasConnection(clientId)) {
      console.log('Does not have a connection for: %s', clientId);

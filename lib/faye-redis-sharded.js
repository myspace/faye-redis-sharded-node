...
Engine.prototype = {
  ...
  publish:function (message, channels) {
    ...
    function publish(message, channels, shardManager) {
      ...
      var allClients = {};
      var shardsToCheck = Object.keys(shardMap).length;
      ...
      function getClientsForChannels(message, channels, channelShard, done) {
        var keys = channels.map(function (c) {
         ...
         // get all clients subscribed to the channels on that shard
      ...
      function checkDone() {
        ...
        if (shardsToCheck == 0) {
          ...
          Object.keys(allClients).forEach(function (clientId) {
      ...
      Object.keys(shardMap).forEach(function (shardName) {
        ...
        getClientsForChannels(message, channels, shard, function(err, clients) {
          if (err) return;
